...
class Westum_Followupemail_Model_Filter extends Mage_Core_Model_Email_Template_Filter
{
    /*
     ...
     *
     * @return string $subject nearest to $from,
     *     counting single back quotes and skipping $subject when it's inside the opened quote
     */
    public function findNearestUnquoted($subject, $source, $from = 0)
    {
        ...
        $currentPos = $from;
        do {
            if (false === $nearestPos = strpos($source, $subject, $currentPos)) {
                return false;
            }
            for (; $currentPos < $nearestPos; $currentPos++) {
                if ('\\' == $char = substr($source, $currentPos, 1)) {
                    $currentPos++;
                } elseif (self::FILTER_QUOTE_UNPROCESSED == $char) {// ignoring special chars
                    $isQuoted = !$isQuoted;
                }
            }
            if (!$isQuoted) {
    ...
    protected function _findParams($value)
    {
        ...
        if ('"' == $char || '\'' == $char) {//if the first char is " then this is a string
            ...
            if (!$value) {
                return substr($op1, 0, strlen($op1) - 1);
            } // the parameter is text in double quotes
...
//         return $operand1.$conditionSign.$operand2;
    ...
    protected function _parseCondition($params)
    {
        ...
        } elseif (!is_array($parsedCondition)) {
            return false;
        }
        ...
        if ($operand1 && !$parsedCondition['op1 is string']) {
            if (isset($this->_templateVars[$operand1])) {
                $operand1 = $this->_templateVars[$operand1];
            } else {
                $operand1 = $this->_getVariable($operand1, $operand1);
            }
        }
        if ($operand2 && !$parsedCondition['op2 is string']) {
            if (isset($this->_templateVars[$operand2])) {
                $operand2 = $this->_templateVars[$operand2];
            } else {
                $operand2 = $this->_getVariable($operand2, $operand2);
            }
        }
        ...
        switch ($conditionSign) {
            ...
            case ' in ' :
                if (!is_array($operand2)) {
                    return $this->reportError('The second operand is not an array');
                } else {
            ...
            case ' is set' :
            if ($operand2) {
                return 'The second operand is not allowed';
            }
            if (!$operand1) {
                return 'The first operand is required';
            }
                ...
                break;
            default :
                return $this->_parseAdditionalConditions($conditionSign, $operand1, $operand2);
        }
        return true;
    }
    ...
    public function endifDirective($directiveName, $directiveParams, $partStartPos, $level)
    {
        ...
        if (1 !== $found) {
            return array('deleteFromPos' => $this->directiveStack[$level][$ifChainStackIds[0]]['startPos']);
        }
    ...
    public function endDependDirective($directiveName, $directiveParams, $partStartPos, $level)
    {
        ...
        if (!$info['result']) {
            return array('deleteFromPos' => $info['startPos']);
        } else {
            return false;
        }
    }
    ...
    */
    protected function _processDirective($value, $partStartPos, $level)
    {
        ...
        $valueStripped = trim(
            substr($value, $lenOpening, strlen($value) - strlen(self::FILTER_DIRECTIVE_CLOSING_BRACKETS) - $lenOpening)
        );
        if (!$valueStripped) {
            return $this->reportError('empty braces \{ \{  \} \}');
        }
        ...
        if ('/' == substr($directiveName, 0, 1)) {
            $directiveName = 'end' . substr($directiveName, 1);
        } // replacing leading '/' with 'end'
        ...
        try {
            if (array_key_exists($directiveName, $this->complexDirectives)) {
                ...
                if (is_callable($callback)) {
                    ...
                    $result = "No such directive '$directiveName'";
                }
            }
        } catch (Exception $e) {
...
// the next two lines were moved here from __construct() due to compatibility issue to Magento version 1.2
        ...
        do {
            ...
            if (((false !== $posOpening) && (false === $posClosing)) || ((false === $posClosing) && $level)) {

                $this->reportError('non-closed brackets on ' . $level . ' levels');
                break;
            }
            if ((false === $posClosing) && (false === $posOpening)) {
                break;
            }
            if ((false !== $posOpening) && $posOpening < $posClosing) {
                ...
                $directiveResult = $this->_processDirective(
                    substr($value, $partStartPos, $posClosing - $partStartPos), $partStartPos, $level
                );
                if (is_array($directiveResult)) {
                    if (isset($directiveResult['insertFromPos']) && isset($directiveResult['insertLen'])) {
                        $partToInsert = substr(
                            $value, $directiveResult['insertFromPos'], $directiveResult['insertLen']
                        );
                    } else {
                        $partToInsert = '';
                    }

                    $deleteFromPos = isset($directiveResult['deleteFromPos']) ? $directiveResult['deleteFromPos']
                        : 999999;

                    $res = substr($value, 0, $deleteFromPos) . $partToInsert;
                } elseif (is_string($directiveResult)) {
                    // substr($value, 0, $partStartPos) is the part before directive
                    $res = substr($value, 0, $partStartPos) . $directiveResult;
                } elseif (is_object($directiveResult) && method_exists($directiveResult, '__toString')) {
                    $res = substr($value, 0, $partStartPos) . $directiveResult;
                } else {
                    $res = substr($value, 0, $partStartPos);
                }
    ...
    {
        if (isset($this->_templateVars['order']) && is_object($this->_templateVars['order'])
            && $this->_templateVars['order']->getData('order_currency_code')
        ) {
            Mage::app()->getStore($this->getStoreId())->setCurrentCurrencyCode(
                $this->_templateVars['order']->getData('order_currency_code')
            );
        }
        if (isset($this->_templateVars['quote']) && is_object($this->_templateVars['quote'])
            && $this->_templateVars['quote']->getData('quote_currency_code')
        ) {
            Mage::app()->getStore($this->getStoreId())->setCurrentCurrencyCode(
                $this->_templateVars['quote']->getData('quote_currency_code')
            );
    ...
    public function modifierFormatDateTime($value)
    {
        ...
        if (is_numeric($value)) {
            return date($formatStr, $value);
        }

        $date = date_parse($value);
        if (false === $date
            || (isset($date['error_count']) && $date['error_count'])
        ) {
            return $value;
        }
    ...
    public function foreachDirective($construction)
    {
        ...
        if (is_string($var)) {
            ...
            return $this->reportError('The object can not be iterated');
        }
        ...
        $paramName = (isset($params[self::ROW_ITEM_ALIAS_NAME]) && $params[self::ROW_ITEM_ALIAS_NAME])
            ? $params[self::ROW_ITEM_ALIAS_NAME] : self::ROW_ITEM_NAME;
        ...
        foreach ($var as $varItem) {
            ...
            $this->_templateVars[$paramName . self::ROW_NUMBER_SUFFIX] = (string)$rowNumber;
        ...
        if (isset($params[self::THUMBNAIL_SOURCE]) && $params[self::THUMBNAIL_SOURCE]) {
            ...
            if (is_object($source)) {
                if ($source instanceof Mage_Catalog_Model_Product) {
                    ...
                    return $this->reportError('The object specified is not a product to take a thumbnail from');
                }
                ...
                return $this->reportError('Wrong object type');
            }
        } else {
            return $this->reportError(
                'No source parameter is specified; there is nowhere to take the thumbnail from'
            );
        }

        $imgDimension = isset($params[self::THUMBNAIL_DIMENSION]) ? $params[self::THUMBNAIL_DIMENSION]
            : self::THUMBNAIL_DIMENSION_DEFAULT;

        return Mage::getModel('core/url')->getUrl(
            'followupemail/index/image',
            array(
    ...
    public function exprDirective($construction)
    {
        ...
        if (3 == count($parts)) {
            ...
            return $this->reportError('Operand(s) or operation sign missed!');
        }
        ...
        switch ($operation) {
            case '-':
                if (is_numeric($operand1) && is_numeric($operand2)) {
                    ...
                    return $this->reportError('You cannot subtract strings!');
                }
                break;
            ...
            case '*':
                if (is_numeric($operand1) && is_numeric($operand2)) {
                    ...
                    return $this->reportError('You cannot multiply strings!');
                }
                break;
            ...
            case '/':
                if (is_numeric($operand1) && is_numeric($operand2)) {
                    if ($operand2) {
                        ...
                        return $this->reportError('Division by zero!');
                    }
                } else {
                    return $this->reportError('You cannot divide strings!');
                }
                break;
            case '%':
                if (is_numeric($operand1) && is_numeric($operand2)) {
            ...
            case 'round':
                if (is_numeric($operand1) && is_numeric($operand2)) {
                    ...
                    return $this->reportError('You cannot round strings!');
                }
                break;
            default:
    ...
    public function setDirective($construction)
    {
        ...
        if ($value
            && in_array(
                $openingQuote = substr($value, 0, 1), array('\'', '"', self::FILTER_QUOTE_UNPROCESSED)
            )
        ) {
