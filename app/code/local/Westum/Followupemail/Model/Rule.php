...
class Westum_Followupemail_Model_Rule extends Mage_Core_Model_Abstract
{
    ...
    {
        $this->_categories = Westum_Followupemail_Helper_Data::noEmptyValues(explode(',', $this->getCategoryIds()));
        $this->_stores = Westum_Followupemail_Helper_Data::noEmptyValues(explode(',', $this->getStoreIds()));
        $this->_productTypes = Westum_Followupemail_Helper_Data::noEmptyValues(explode(',', $this->getProductTypeIds()));
        $this->_products = Westum_Followupemail_Helper_Data::noEmptyValues(explode(',', $this->getProductIds()));
        $this->_sku = Westum_Followupemail_Helper_Data::noEmptyValues(explode(',', $this->getSku()));
        $this->_anlSegments = Westum_Followupemail_Helper_Data::noEmptyValues(explode(',', $this->getAnlSegments()));
        $this->_customerGroups = Westum_Followupemail_Helper_Data::noEmptyValues(explode(',', $this->getCustomerGroups()));

        $len = strlen(Westum_Followupemail_Model_Source_Rule_Types::RULE_TYPE_ORDER_STATUS_PREFIX);
        $this->_orderStatus = (Westum_Followupemail_Model_Source_Rule_Types::RULE_TYPE_ORDER_STATUS_PREFIX ==
        ...
        if ($this->getSendToSubscribersOnly()) {
            ...
            if (!$subscriber
                ...
                if (Mage::helper('followupemail')->canUseAN()) {
                    ...
                    if ($subscriber
                        && $subscriber->getData()
                        && $subscriber->getStatus() == Westum_Advancednewsletter_Model_Subscriber::STATUS_SUBSCRIBED
                    ) {
                        return true;
                    }
                    return false;
    ...
    protected function _validateByAdvancedNewsletterSegments($email)
    {
        if ($this->getSendToSubscribersOnly()
            ...
            && !in_array(Westum_Followupemail_Model_Mysql4_Rule::ADVANCED_NEWSLETTER_SEGMENTS_ALL, $this->_anlSegments)
    ...
    protected function _validateByCustomerGroup($customer)
    {
        if (!count($this->_customerGroups)
            || in_array(Westum_Followupemail_Model_Source_Customer_Group::CUSTOMER_GROUP_ALL, $this->_customerGroups)
        ) {
            return true;
        }
        ...
        if ($customer && $customer->getId()) {
            ...
            Westum_Followupemail_Model_Source_Customer_Group::CUSTOMER_GROUP_NOT_REGISTERED, $this->_customerGroups
        )
        ) {
    ...
    public function validateByCustomer($params)
    {
        ...
        if (true !== $result = $this->_validateByAdvancedNewsletterSegments($params['customer_email'])) {
            return $result;
        }

        if (true !==
            $result = $this->_validateByCustomerGroup(isset($params['customer']) ? $params['customer'] : false)
        ) {
            return $result;
        }
    ...
    {
        $parts = explode(Westum_Followupemail_Model_Source_Rule_Saleamount::CONDITION_SEPARATOR, $this->getSaleAmount(), 2);
        if (count($parts) < 2) {
            return true;
        }
        ...
        switch (array_search(
            $condition, Westum_Followupemail_Model_Source_Rule_Saleamount::getConditions()
        )) // switch($condition)
        {
            case Westum_Followupemail_Model_Source_Rule_Saleamount::CONDITION_EQ  :
                $result = ($saleAmount == $value);
                break;
            case Westum_Followupemail_Model_Source_Rule_Saleamount::CONDITION_GT  :
                $result = ($saleAmount > $value);
                break;
            case Westum_Followupemail_Model_Source_Rule_Saleamount::CONDITION_EGT :
                $result = ($saleAmount >= $value);
                break;
            case Westum_Followupemail_Model_Source_Rule_Saleamount::CONDITION_LT  :
                $result = ($saleAmount < $value);
                break;
            case Westum_Followupemail_Model_Source_Rule_Saleamount::CONDITION_ELT :
                $result = ($saleAmount <= $value);
                break;
            case Westum_Followupemail_Model_Source_Rule_Saleamount::CONDITION_NE  :
                $result = ($saleAmount != $value);
                break;
            default :
    ...
    {
        if (true !== ($result = $this->validateByProduct($params))) {
            return $result;
        }
    ...
    public function validateByProduct($params)
    {
        ...
        if (isset($params['product_type_ids'])) {
            $params['product_type_ids'] = Westum_Followupemail_Helper_Data::noEmptyValues($params['product_type_ids']);
        }
        if (isset($params['category_ids'])) {
            $params['category_ids'] = Westum_Followupemail_Helper_Data::noEmptyValues($params['category_ids']);
        }
        if (isset($params['product_ids'])) {
            $params['product_ids'] = Westum_Followupemail_Helper_Data::noEmptyValues($params['product_ids']);
        }

        if (count($this->_productTypes)
            && !in_array(Westum_Followupemail_Model_Source_Product_Types::PRODUCT_TYPE_ALL, $this->_productTypes)
            && isset($params['product_type_ids'])
            && count($params['product_type_ids'])
        ) {
            ...
            if (!$productTypeCondition) {
                return 'Product type(s) doesn\'t match';
            }
        }

        // category rule
        if (count($this->_categories)
            && isset($params['category_ids'])
            && count($params['category_ids'])
        ) {
        ...
        if (count($this->_productTypes)
            && !in_array(
                Westum_Followupemail_Model_Source_Product_Types::PRODUCT_TYPE_ALL, $this->_productTypes
            )
            && isset($params['product_type_ids'])
            && count($params['product_type_ids'])
            && count($this->_sku)
            ...
            if (!$bothSKUAndTypeCondition) {
                return 'Products SKU\'s and types doesn\'t match';
            }
        }
        return true;
    }
    ...
    /*
     * Validates rule basing on parameters passed by invoking internal _validate
     * function and puts validation into internal validation message variable
     * @param array $params Parameters to inspect
     * @return bool Check result
     */
    ...
    protected function _validate($params)
    {
        ...
        switch ($this->getEventType()) {
            case Westum_Followupemail_Model_Source_Rule_Types::RULE_TYPE_WISHLIST_SHARED :
            case Westum_Followupemail_Model_Source_Rule_Types::RULE_TYPE_WISHLIST_PRODUCT_ADD :
                return $this->validateByProduct($params);
                break;

            case Westum_Followupemail_Model_Source_Rule_Types::RULE_TYPE_ABANDONED_CART_NEW :
                if ($mssRuleId
                    && isset($params['quote'])
            ...
            case Westum_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_NEW :
            case Westum_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_LOGGED_IN :
            case Westum_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_LAST_ACTIVITY :
            case Westum_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_BIRTHDAY :
            case Westum_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_NEW_SUBSCRIPTION :
                // return $this->validateByCustomer($params);
                return true;
                break;

            case Westum_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_CAME_BACK_BY_LINK :
                // return $this->validateByCustomer($params);
                return true;
                break;

            case Westum_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_GROUP_CHANGED:
                return $this->validateByCustomer($params);
                break;
     ...
     * Called from Westum_Followupemail_Model_Filter::foreachDirective()
    ...
    protected function _getTemplate($templateId, $storeId = null)
    {
        ...
        $templateName = substr(
            $templateId,
            false !== (
            $pos = strpos($templateId, Westum_Followupemail_Model_Source_Rule_Template::TEMPLATE_SOURCE_SEPARATOR))
            ? $pos + 1 : 0
        );
        ...
        if (!$pos) {
            ...
            switch ($src = substr($templateId, 0, $pos)) {
                case Westum_Followupemail_Model_Source_Rule_Template::TEMPLATE_SOURCE_EMAIL :
                    $template = $this->getResource()->getTemplateContent('core/email_template', $templateName);
                    break;

                case Westum_Followupemail_Model_Source_Rule_Template::TEMPLATE_SOURCE_NEWSLETTER :
            ...
            $template['sender_name'] = $this->getData('sender_name') ? $this->getData('sender_name')
                : Mage::getStoreConfig("trans_email/ident_$sender/name", $storeId);
            $template['sender_email'] = $this->getData('sender_email') ? $this->getData('sender_email')
                : Mage::getStoreConfig("trans_email/ident_$sender/email", $storeId);
    ...
    {
        if (!$content = $this->_getTemplate($templateId, $objects['store_id'])) {
            return false;
        }

        if ($this->getSenderName()) {
            $content['sender_name'] = $this->getSenderName();
        }
        if ($this->getSenderEmail()) {
            $content['sender_email'] = $this->getSenderEmail();
        }
        ...
        if (count($this->_getProcessor()->getErrors()) > 0) {
            return false;
        }
    ...
    protected function _createCustomer($objects)
    {
        if (!isset($objects['customer'])) {
            if (isset($objects['customer_id']) && $objects['customer_id']) {
            ...
            } elseif (isset($objects['order']) && $objects['order'] instanceof Mage_Sales_Model_Order) {
                $billingAddress = Westum_Followupemail_Helper_Data::getOrderAddress($objects['order'], 'billing');
                $objects['customer'] = Mage::getModel('customer/customer');
                $objects['customer']->setFirstname($billingAddress->getFirstname());
                $objects['customer']->setLastname($billingAddress->getLastname());
            } elseif (isset($objects['quote']) && $objects['quote'] instanceof Mage_Sales_Model_Quote) {
                $objects['customer'] = Mage::getModel('customer/customer');
                $objects['customer']->setFirstname($objects['quote']->getCustomerFirstname());
                $objects['customer']->setLastname($objects['quote']->getCustomerLastname());
                ...
                if (!$objects['customer_id']) {
                    $objects['customer'] = Mage::getModel('customer/customer')->setData(
                        array(
                             'id'       => null,
                             'group_id' => 0,
                        )
        ...
        if (isset($objects['customer'])) {
            ...
            if (!isset($objects['customer_email'])) {
                $objects['customer_email'] = $objects['customer']->getEmail();
            }
        } elseif (isset($objects['order'])) {
            if (!isset($objects['customer_email'])) {
                $objects['customer_email'] = $objects['order']->getCustomerEmail();
            }
            $infoSource = Westum_Followupemail_Helper_Data::getOrderAddress($objects['order'], 'billing');
            if (!$infoSource) {
                $infoSource = Westum_Followupemail_Helper_Data::getOrderAddress($objects['order'], 'shipping');
            }
        } elseif (isset($objects['quote']) && $objects['quote'] instanceof Mage_Sales_Model_Quote) {
        ...
        if (false != $infoSource) {
            ...
            $objects['customer_name']
                =
                $infoSource->getFirstname() . ' ' . ($middlename ? $middlename . ' ' : '') . $infoSource->getLastname();
        }
        if (!isset($objects['customer_name'])) {
            if (isset($objects['customer_email'])) {
                $objects['customer_name'] = substr(
                    $objects['customer_email'], 0,
                    false !== ($pos = strpos($objects['customer_email'], '@')) ? $pos : 999
                );
            } else {
                $objects['customer_name'] = 'Friend';
            }
        }
    ...
    protected function _createObjects($params, $objects = array())
    {
        ...
        if (!isset($objects['order'])) {
            if (isset($objects['order_id']) && $objects['order_id']) {
                ...
                $objects['order'] = Mage::getModel('sales/order')->reset()->loadByIncrementId(
                    $objects['order_increment_id']
                );
            }
        }
        if (isset($objects['order']) && !$objects['order']->getId()) {
            unset($objects['order']);
        }
        if (isset($objects['order'])) {
            if (!isset($objects['customer_id'])) {
                $objects['customer_id'] = $objects['order']->getCustomerId();
            }
            ...
            foreach ($objects['order']->getAllVisibleItems() as $item) {
                ...
                if ($item->getProductType() == Mage_Catalog_Model_Product_Type_Grouped::TYPE_CODE) {
                    ...
                    if (is_array($buyRequest)) {
                        $parentProductId = $buyRequest['info_buyRequest']['super_product_config']['product_id'];
                        $product->setId($parentProductId);
                    }
                }
                $objects['order']->getItemById($item->getId())->setProduct($product);
            }
        }
        ...
        if (isset($objects['quote'])) {
            if (!$objects['quote']->getId()) {
                ...
                $objects['quote']->setTotalDiscount(
                    $objects['quote']->getSubtotal() - $objects['quote']->getSubtotalWithDiscount()
                );
                ...
                foreach ($objects['quote']->getAllVisibleItems() as $item) {
                    ...
                    $objects['quote']->getItemById($item->getId())->setProduct($product);
                }
        ...
        if (!isset($objects['wishlist'])) {
            if (isset($objects['wishlist_id']) && $objects['wishlist_id']) {
                ...
                $objects['wishlist'] = Mage::getModel('wishlist/wishlist')->loadByCode(
                    $objects['wishlist_sharing_code']
                );
        ...
        if (isset($objects['wishlist'])) {
            foreach ($objects['wishlist']->getItemCollection() as $item) {
                $objects['wishlist']->getItemCollection()->getItemById($item->getId())->setProduct(
            ...
            if (!isset($objects['store_id']) || !$objects['store_id']) {
                if (isset($objects['customer']) && $objects['customer']) {
                    ...
                    if ($objects['store_id'] == 0) {
                        $storeIds = Mage::getModel('core/website')->load($objects['customer']->getWebsiteId())
                            ->getStoreIds();
                        if ($storeIds) {
                ...
                } elseif (isset($objects['quote'])
                    && $objects['quote']
                ) {
                    $objects['store_id'] = $objects['quote']->getStoreId();
                } elseif (isset($objects['wishlist'])
                    && $objects['wishlist']
                ) {
        ...
        if (Mage::helper('followupemail')->checkVersion('1.9') && isset($objects['customer'])
            && $objects['customer'] instanceof Mage_Customer_Model_Customer
        ) {
    ...
    {
        if (isset($objects['order'])) {
            $objectForCross = $objects['order'];
            $quoteId = $objects['order']->getQuoteId();
        } elseif (isset($objects['cart'])) {
            $objectForCross = $objects['cart'];
            $quoteId = $objects['cart']->getId();
        } else {
            return '';
        }
        ...
        if ($source == Westum_Followupemail_Model_Source_Rule_Cross::MAGENTO_RELATED) {
            foreach ($objectForCross->getAllVisibleItems() as $item) {
                $boughtProductIds = array_merge($item->getProduct()->getRelatedProductIds(), $boughtProductIds);
                $orderProductIds[] = $item->getProduct()->getId();
            }
        }
        if ($source == Westum_Followupemail_Model_Source_Rule_Cross::MAGENTO_CROSS) {
            foreach ($objectForCross->getAllVisibleItems() as $item) {
        ...
        if ($source == Westum_Followupemail_Model_Source_Rule_Cross::MAGENTO_UPSELLS) {
            foreach ($objectForCross->getAllVisibleItems() as $item) {
                $boughtProductIds = array_merge($item->getProduct()->getUpSellProductIds(), $boughtProductIds);
                $orderProductIds[] = $item->getProduct()->getId();
            }
        }
        if ($source == Westum_Followupemail_Model_Source_Rule_Cross::Westum_WBTAB) {
            //check for availability
            if (Mage::helper('followupemail')->isWBTABInstalled()) {
                foreach ($objectForCross->getAllVisibleItems() as $item) {
                    $orderProductIds[] = $item->getProduct()->getId();
                }
                $boughtProductIds = Mage::getModel('relatedproducts/api')->getRelatedProductsFor(
                    $orderProductIds, $objectForCross->getStoreId()
                );
        ...
        if ($source == Westum_Followupemail_Model_Source_Rule_Cross::Westum_ARP2) {
            //check for availability
            if (class_exists('Westum_Autorelated_Model_Api') && Mage::helper('followupemail')->isARP2Installed()) {
                if ($objects['customer'] == null && $objectForCross) {
                    $objects['customer'] = Mage::getModel('customer/customer')->setData(
                        array(
                             'id'       => $objectForCross->getCustomerId(),
                             'group_id' => $objectForCross->getCustomerGroupId(),
                        )
                    );
                }
                $arp2Collection = Mage::getModel('awautorelated/blocks')->getCollection()
                    ->addCustomerGroupFilter($objects['customer']->getGroupId())
                    ->addTypeFilter(Westum_Autorelated_Model_Source_Type::SHOPPING_CART_BLOCK)
                    ->addStatusFilter()
                    ->addDateFilter()
                ...
                if (count($ids) > 0) {
                    foreach ($ids as $arp2Block) {
                        $boughtProductIds = array_merge(
                            $boughtProductIds,
                            Mage::getModel('awautorelated/api')->getRelatedProductsForShoppingCartRule(
                                $arp2Block, $quoteId
                            )
                        );
                    }
        ...
        Mage::app()->getWebsite()->setId(Mage::app()->getStore($objectForCross->getStoreId())->getWebsiteId());

        $oldStore = Mage::app()->getStore();
        $store = Mage::getModel('core/store')->load($objectForCross->getStoreId());
        $oldCurrency = $store->getCurrentCurrencyCode();
        $store->setCurrentCurrencyCode($objectForCross->getOrderCurrencyCode());
        Mage::app()->setCurrentStore($store);

        $design = Mage::getDesign();
        $oldValues = $design->setAllGetOld(
            array(
                'store' => $objects['store'],
        ...
        $relProductCollection = Mage::getModel('catalog/product')->setStoreId($objectForCross->getStoreId())
            ->getCollection();
        $relProductCollection->addAttributeToSelect('thumbnail')
            ...
            ->addPriceData(
                $objectForCross->getCustomerGroupId(),
                Mage::app()->getStore($objectForCross->getStoreId())->getWebsiteId()
            )
    ...
    public function process($params, $objects = array())
    {
            ...
            foreach ($chain as $chainItem) {
                ...
                if ($this->getCouponEnabled()) {
                    ...
                    if (preg_match_all($pattern1, $emailTemplateContent, $matches) > 0) {
                        ...
                        // using object for access to variables from Westum_Followupemail_Model_Filter::filter()
                ...
                $code = Westum_Followupemail_Helper_Data::getSecurityCode();
        ...
        Mage::getSingleton('followupemail/log')->logWarning(
            ...
            . $this->_validationMessage . '" objectId=' . (isset($objects['object_id']) ? $objects['object_id']
                : 'none') . ', params="' . Westum_Followupemail_Helper_Data::printParams($params), $this
        );
    ...
    public function processBirthday($params, $templateId, $timeDelay, $sequenceNumber)
    {
        ...
        if ($this->_isValid) {
            if (!(isset($params['customer_id']) && $params['customer_id'])) {
                $message
                    =
                    'rule id=' . $this->getId() . ' processing error: customer_id is not set on birthday event, email="'
                    . $objects['customer_email'] . '"';
            ...
            if (!$content = $this->_getContent($objects, $templateId)) {
                $message = "rule id={$this->getId()} has invalid templateId=" . $templateId
                    . " in sequenceNumber=$sequenceNumber"
                ;
                $subject = "Rule has invalid";
                Mage::getSingleton('followupemail/log')->logError($message, $this, $subject);
        ...
        Mage::getSingleton('followupemail/log')->logWarning(
            "rule id={$this->getId()} is not valid for birthday event of customerId={$params['customer_id']}, params="
            . Westum_Followupemail_Helper_Data::printParams($params), $this
        );
        return false;
    }
    ...
    */
    public function sendTestEmail($params, $objects = array())
    {
        ...
        $params = $this->_fillTestObjectValues($params, $this->getEventType());
        ...
        if (Westum_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_BIRTHDAY == $this->getEventType()) {
            ...
            foreach (unserialize($this->getChain()) as $chain) {
                $result
                    =
                    $result && $this->processBirthday($params, $chain['TEMPLATE_ID'], $chain['DAYS'], $sequenceNumber);
                $sequenceNumber++;
            }
        } else {
    ...
    protected function _fillTestObjectValues($params, $eventType)
    {
        if (strpos($eventType, Westum_Followupemail_Model_Source_Rule_Types::RULE_TYPE_ORDER_STATUS_PREFIX) > -1) {
            if (empty($params['order_increment_id'])) {
                $order = $this->_getRandomOrder();
                $params['order_increment_id'] = $order->getIncrementId();
            }
            if (empty($params['customer_id'])) {
                $order = Mage::getModel('sales/order')->loadByIncrementId($params['order_increment_id']);
                if ($order->getCustomerId()) {
                    $params['customer_id'] = $order->getCustomerId();
                } else {
                    $customer = $this->_getRandomCustomer();
                    $params['customer_id'] = $customer->getEntityId();
                }
            }
            ...
            switch($eventType) {
                case Westum_Followupemail_Model_Source_Rule_Types::RULE_TYPE_ABANDONED_CART_NEW :
                    if (empty($params['quote_id'])) {
                        ...
                        $params['quote_id'] = $quote->getEntityId();
                        if (empty($params['customer_id'])) {
                            $quote = Mage::getModel('sales/quote')->load($params['quote_id']);
                            if ($quote->getCustomerId()) {
                                $params['customer_id'] = $quote->getCustomerId();
                            } else {
                                $customer = $this->_getRandomCustomer();
                                $params['customer_id'] = $customer->getEntityId();
                            }
                ...
                case Westum_Followupemail_Model_Source_Rule_Types::RULE_TYPE_WISHLIST_SHARED:
                case Westum_Followupemail_Model_Source_Rule_Types::RULE_TYPE_WISHLIST_PRODUCT_ADD:
                    if (empty($params['wishlist_id'])) {
                        $wishlist = $this->_getRandomWishlist();
                        $params['wishlist_id'] = $wishlist->getId();
                        if (empty($params['customer_id'])) {
                            $wishlist = Mage::getModel('wishlist/wishlist')->load($params['wishlist_id']);
                    ...
                    if (empty($params['product_id'])) {
                        ...
                        $params['product_id'] = $product->getEntityId();
                    }
                    break;
                case Westum_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_NEW :
                case Westum_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_LOGGED_IN :
    ...
    protected function _getRandomOrder()
    {
        $coll = Mage::getModel('sales/order')->getCollection();
        $coll->getSelect()
            ->reset(Zend_Db_Select::COLUMNS)
            ->columns(array('entity_id', 'increment_id', 'customer_id', 'customer_email'))
    ...
    protected function _getRandomQuote()
    {
        ...
        $coll->getSelect()
            ->reset(Zend_Db_Select::COLUMNS)
            ->columns(array('entity_id', 'customer_id', 'customer_email'))
            ->order(new Zend_Db_Expr('RAND()'))
            ->where('items_count > ?', 0)
            ->limit(1)
        ;
    ...
    protected function _getRandomWishlist()
    {
        $coll = Mage::getModel('wishlist/wishlist')->getCollection();
        $coll->getSelect()
            ->reset(Zend_Db_Select::COLUMNS)
            ->columns(array('wishlist_id', 'customer_id'))
            ->order(new Zend_Db_Expr('RAND()'))
            ->limit(1)
        ;
        return $coll->getFirstItem();
