...
class Westum_Followupemail_Adminhtml_RulesController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function editAction()
    {
        ...
        if (!empty($data) || $ruleId == 0) {
            ...
            if (!isset($data['customer_groups'])
                ...
                || (in_array(Westum_Followupemail_Model_Source_Customer_Group::CUSTOMER_GROUP_ALL, $data['customer_groups'])
                    && count($data['customer_groups']) > 1)
            ) {
                $data['customer_groups'] = array(Westum_Followupemail_Model_Source_Customer_Group::CUSTOMER_GROUP_ALL);
            }
            ...
            if (Mage::helper('followupemail')->canUseAN()) {
                if (!isset($data['anl_segments'])
                    || !$data['anl_segments']
                    || !count(
                        is_array($data['anl_segments']) ? $data['anl_segments']
                        : ($data['anl_segments'] = explode(',', $data['anl_segments']))
                    )
                    || (
                        in_array(
                            Westum_Followupemail_Model_Mysql4_Rule::ADVANCED_NEWSLETTER_SEGMENTS_ALL, $data['anl_segments']
                        )
                        && count($data['anl_segments']) > 1
                    )
                ) {
                    $data['anl_segments'] = array(Westum_Followupemail_Model_Mysql4_Rule::ADVANCED_NEWSLETTER_SEGMENTS_ALL);
                }
            ...
            if (!isset($data['sale_amount_condition'])
                ...
                && is_array(
                    $saleAmount = explode(
                        Westum_Followupemail_Model_Source_Rule_Saleamount::CONDITION_SEPARATOR, $data['sale_amount'], 2
                    )
                )
                && count($saleAmount) > 1
            ) {
                $data['sale_amount_condition'] = array_search(
                    $saleAmount[0], Westum_Followupemail_Model_Source_Rule_Saleamount::getConditions()
                );
    ...
    public function saveAction($sendTest = false)
    {
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if (!isset($data['product_type_ids']) || in_array('all', $data['product_type_ids'])) {
                $data['product_type_ids'] = Westum_Followupemail_Model_Source_Product_Types::PRODUCT_TYPE_ALL;
            } else {
            ...
            if ($data['active_from']) {
                ...
                try {
                    $dateFrom = Mage::app()->getLocale()->utcDate(null, $data['active_from'], true, $formatDate)
            ...
            if ($data['active_to']) {
                ...
                try {
                    $date = Mage::app()->getLocale()->utcDate(null, $data['active_to'], true, $formatDate)->toString(
            ...
            if ($data['sale_amount_value'] || $data['sale_amount_condition']) {
                $data['sale_amount']
                    = Westum_Followupemail_Model_Source_Rule_Saleamount::getCondition($data['sale_amount_condition'])
                    . Westum_Followupemail_Model_Source_Rule_Saleamount::CONDITION_SEPARATOR . $data['sale_amount_value'];
            } else {
                $data['sale_amount'] = '';
            }
            ...
            if (!isset($data['chain'])) {
                ...
                foreach ($data['chain'] as $key => $value) {
                    if (false ===
                        strpos(
                            $value['TEMPLATE_ID'],
                            Westum_Followupemail_Model_Source_Rule_Template::TEMPLATE_SOURCE_SEPARATOR
                        )
                    ) {
                ...
                if (count($data['chain']) > 1) {
                    ...
                    foreach ($data['chain'] as $k => $v) {
                        $chainSorted[
                        $v['BEFORE'] * ($v['DAYS'] * 1440 + $v['HOURS'] * 60 + $v['MINUTES']) * 10000 + mt_rand(
                            0, 9999
                        )]
                            = $k;
                    }
            ...
            if ($this->getRequest()->getParam('coupon_enabled')) {
                ...
                if (!$couponExpireDays || !intval($couponExpireDays)) {
                    $session->addError(
                        $this->__('Coupon expire days value must be integer and equals or greater than 1')
                    );
                    $session->setFollowupemailData($data);
                    return $this->_redirect(
                        '*/*/edit',
                        array('id' => $this->getRequest()->getParam('id'), 'tab' => 'followupemail_tabs_coupons')
                    );
                }
                ...
                if ($couponCode && !preg_match('/^[a-zA-Z0-9]*$/', $couponCode)) {
                    $session->addError(
                        $this->__(
                            'The following symbols are allowed to be used in the \'Coupon Code Prefix\' field: a-z 0-9'
                        )
                    );
                    $session->setFollowupemailData($data);
                    return $this->_redirect(
            ...
            /** @var $model Westum_Followupemail_Model_Rule */
        ...
        if (!is_array($followupemailIds)) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    $this->__('Total of %d record(s) were successfully deleted', count($followupemailIds))
                );
            } catch (Exception $e) {
                Mage::logException($e);
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
        ...
        if (!is_array($followupemailIds)) {
            ...
            try {
                ...
                $this->_getSession()->addSuccess(
                    $this->__('Total of %d record(s) were successfully updated', count($followupemailIds))
                );
            } catch (Exception $e) {
                Mage::logException($e);
